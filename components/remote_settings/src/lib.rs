...
impl RemoteSettingsService {
    ...
    pub fn make_client(&self, collection_name: String) -> Result<Arc<RemoteSettingsClient>> {
        let storage = Storage::new(&Utf8Path::new(&self.config.storage_dir).join(format!("{}.sqlite", collection_name)))?;
        let client = Client::new_for_remote_settings_service(
            self.config.server.clone(),
            self.config.bucket_name.clone().unwrap_or_else(|| "main".to_string()),
            collection_name,
        )?;
...
impl RemoteSettingsClient {
    ...
    pub fn sync(&self) -> Result<()> {
        ...
        self.storage.update_records(resp.records, resp.last_modified)?;
        Ok(())
    }
