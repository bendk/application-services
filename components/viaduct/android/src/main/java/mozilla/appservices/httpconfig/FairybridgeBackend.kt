...
import mozilla.appservices.fairy_bridge.Backend
import mozilla.appservices.fairy_bridge.Method
import mozilla.appservices.fairy_bridge.Request
import mozilla.appservices.fairy_bridge.Response
...
internal class FairyBridgeBackend(val client: Lazy<FetchClient>): Backend {
    override suspend fun sendRequest(request: Request): Response {
        val fetchReq = FetchRequest(
            ...
            headers = FetchMutableHeaders(request.headers.map {
                (name, value) -> FetchHeader(name, value)
            }),
            ...
            // Try to honor the request settings as best we can using the Fetch API
            readTimeout = if (request.settings.timeout > 0UL) {
                Pair(request.settings.timeout.toLong(), TimeUnit.MICROSECONDS)
            } else {
                null
            },
            redirect = if (request.settings.redirectLimit.toInt() > 0) {
